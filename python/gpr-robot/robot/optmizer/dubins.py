...
# Headins in radians, always radians!
...
class Planner:
    def __init__(self,radius, point_separation):
        self.r = radius
        self.ds = point_separation
        self.planner = planner(radius, point_separation)    
    def length(self,xs,ys,hs,xe,ye,he):
        return length(self.planner,xs,ys,hs,xe,ye,he)
    def path(self,xs,ys,hs,xe,ye,he):
        x,y = path(self.planner,xs,ys,hs,xe,ye,he)
        h = headings(x,y,hs)
        return x,y,h
    def optimal(self,xs,ys,hs,xe,ye):
        return optimize(self.planner,xs,ys,hs,xe,ye)

#############################################################################################
# Functions  

def planner(radius, point_separation):
    return Dubins(radius, point_separation)

def path(planner, xs, ys, hs, xe, ye, he):
    points = planner.dubins_path((xs,ys,hs),(xe,ye,he))
    return points[:, 0], points[:, 1]

def length(planner,xs,ys,hs,xe,ye,he):
    options = planner.all_options((xs,ys,hs),(xe,ye,he))
    return min(options, key=lambda x: x[0])[0]

def heading(xp,yp,x,y,xn=None,yn=None):
    if (xn is None) or (yn is None): # No next
        h = np.arctan2(y-yp,x-xp)
    else:
        hp = np.arctan2(y-yp,x-xp)
        hn = np.arctan2(yn-y,xn-x)
        h = 0.5*(hp + hn)
    return normalize(h)

def headings(x,y,hs=None):
    n = x.size
    if n < 2:
        if hs is not None:
            return np.array([hs])
            ...
            return np.array([0.0])
    # Previous
    dxp = x[1:-1] - x[0:-2]  # x[i] - x[i-1]
    dyp = y[1:-1] - y[0:-2]  # y[i] - y[i-1]
    hp = np.arctan2(dyp,dxp)
    # Next
    dxn = x[2:] - x[1:-1]    # x[i+1] - x[i]
    dyn = y[2:] - y[1:-1]    # y[i+1] - y[i]
    hn = np.arctan2(dyn, dxn)
    # Average of previous and next headings
    ha = 0.5*(hp + hn)
    # Combine all
    h = np.zeros(n)
    if hs is not None:
        ...
        h[0] = np.arctan2(y[1] - y[0], x[1] - x[0])
    h[1:-1] = ha
    h[-1] =np.arctan2(y[-1] - y[-2], x[-1] - x[-2])
    return normalize(h)
...
# Search best heading [0, 2*pi]
def sample(planner,xs,ys,hs,xe,ye,n=180):
    l = np.zeros(n)
    h = 0.0
    dh = 2*pi/(n-1)
    for i in range(n):
        l[i] = length(planner,xs,ys,hs,xe,ye,h)
        h += dh
    return dh*np.argmin(l)

# Sample best heading [0, 2*pi]
def optimize(planner,xs,ys,hs,xe,ye):
    he = sample(planner,xs,ys,hs,xe,ye)
    bounds = [(0, 2*pi)]

    def objective(he,planner,xs,ys,hs,xe,ye):
        return length(planner,xs,ys,hs,xe,ye,he[0])

    result = scp.optimize.minimize(
        objective, 
        he,
        args=(planner, xs, ys, hs, xe, ye),
        method='L-BFGS-B',
        bounds=bounds
    )
    return result.x[0] 
