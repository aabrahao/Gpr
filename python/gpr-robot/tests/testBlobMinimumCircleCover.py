...
def visualize(x, y, centers_x, centers_y, radius):
    """Visualize the results."""
    plt.figure(figsize=(12, 10))
    plt.title(f'Minimal circles cover (radius={radius})')
    ...
    # Plot circles
    ax = plt.gca()
    for xi, yi in zip(centers_x, centers_y):
        circle = Circle((xi, yi), radius, fill=True, color='blue', alpha=0.2)
        ax.add_patch(circle)
    plt.scatter(centers_x, centers_y, c='blue', s=30, alpha=0.8, label='Circle centers')
...
if __name__ == "__main__":
    ...
    # Run algorithm
    centers_x, centers_y = minimal_circles_cover(x, y, radius)
    
    # Visualize
    visualize(x, y, centers_x, centers_y, radius)